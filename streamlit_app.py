...
def collect_repo_dashboard_data(owner: str, repo: str, branch: str):
    """
    Recupera i dati per il cruscotto.
    ...
    <div class="section">
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div class="card-value">+{{ summary.avg_additions }} / -{{ summary.avg_deletions }}<br><span class="muted">file modificati: {{ summary.avg_files }}</span></div>
            </div>
        </div>
    </div>
...
def make_gantt_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Costruisce task per Gantt.
    Ogni riga con tag valorizzato diventa un task con durata default 2 ore.
    """
    tasks = []
    for _, r in df.iterrows():
        tag = (r.get("Activity Tag") or "").strip()
...
def render_gantt_chart(tasks_df: pd.DataFrame, project_end: dt.date, extension_dates: list):
    """
    Gantt con plotly express timeline.
    Mostra una linea sulla data fine progetto e una fascia evidenziata oltre quella data.
    """
    try:
    ...
    if tasks_df.empty:
        st.info("Compila almeno qualche riga con Activity Tag e Description, poi genera il Gantt.")
        return

    # Ordine più leggibile: start crescente
    ...
    fig.update_layout(
        height=min(900, 120 + 28 * len(tasks_df)),
    ...
    # Fine progetto come linea verticale
    end_dt = dt.datetime.combine(project_end, dt.time(23, 59))
    fig.add_vline(x=end_dt, line_width=2, line_dash="dot", line_color="#ef4444")

    # Evidenzia area oltre fine progetto fino alla massima estensione (o fino all’ultimo task)
...
def main():
    ...
    with tab_pan:
        ...
        if dashboard_data["commit_weeks"]:
            ...
            df_weeks = df_weeks.rename(columns={"label": "Settimana", "total": "Commit"}).set_index("Settimana")
        ...
        if dashboard_data["commits"]:
            df_commits = pd.DataFrame(dashboard_data["commits"]).rename(
                columns={"sha": "SHA", "message": "Messaggio", "author": "Autore", "date_display": "Data"}
    ...
    with tab_author:
            ...
            if st.button("Calcola vista 360 autore"):
                ...
                if commits:
                    ...
                    df_changes_chart = df_changes[["date_display", "additions", "deletions"]].rename(
                        columns={"date_display": "Data", "additions": "Righe aggiunte", "deletions": "Righe rimosse"}
                    ).set_index("Data")
        ...
        # ============================================================
        # Nuovo blocco: Responsabile del progetto
        # ============================================================
        ...
        saved_map = saved.get("commit_inputs", {})
        base_df = merge_saved_inputs(base_df, saved_map)

        # Data editor con scroll e colonne input
        edited = st.data_editor(
            ...
            column_config={
                "Activity Tag": st.column_config.SelectboxColumn(
                    "Activity Tag",
                    options=[""] + ACTIVITY_TAG_OPTIONS,
                    required=False,
                ),
                "Activity Description": st.column_config.TextColumn(
                    "Activity Description",
                    help="Descrizione sintetica dell’attività svolta per questo commit",
                    required=False,
                ),
        ...
        if st.button("Genera Gantt chart", key=f"pm_gantt_{repo_key}"):
            ...
            st.caption("Suggerimento: l’area evidenziata dopo la data fine progetto rappresenta la fase in estensione.")
